...
| API5 | Broken function level authorization | Surface is exactly `POST /chat` + `GET /health`; no admin/debug routes; FastAPI docs disabled (`docs_url=None, redoc_url=None`), so no unintended function surface exists to authorize. | `create_app` source; implicit (no other routes) |
...
| API9 | Improper inventory | Inventory is exactly two routes, kept in `src/telco_churn/api/app.py` and this document + `docs/architecture.md`; both are short-lived demo endpoints. | This file; spec `chat-api` inventory clause |
...
Compliance status: every scenario in
`openspec/changes/add-telco-churn-poc/specs/telco-churn/chat-api/spec.md`
is traceable to a test in `tests/unit/test_chat_api.py` or a control
documented above; the performance-budget numbers are recorded separately
in `docs/perf-report.md`.

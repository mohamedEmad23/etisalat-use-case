...
- Extract-only chat pipeline: Qwen3-4B-Instruct-2507 (Ollama tag `qwen3:4b-instruct-2507-q4_K_M`) emits grammar-constrained feature requests (no numeric fields in schema); hand-rolled ~150-line tool loop; numerics string-interpolated from tool output only; curated extraction test suite (100% schema validity, ≥ 95% slot accuracy).

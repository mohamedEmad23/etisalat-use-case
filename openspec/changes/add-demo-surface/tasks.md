...
- [x] 2.1 Create `deploy/modal_app.py` — single-container pattern: one GPU `@modal.web_server(port=8000)` function (T4 default, L4 via env) running `ollama serve` on 127.0.0.1:11434, readiness wait, pull-if-missing `ollama pull` of the configured model, then uvicorn on 8000; weights in `modal.Volume("ollama-models")` → `/root/.ollama`, artifacts volume → `/srv/app/runs`, bearer token via `modal.Secret`, `scaledown_window=1800`. Verify: `python -c "import ast; ast.parse(open('deploy/modal_app.py').read())"` parses; dry structure matches design D4; file marked "ready, not exercised — requires user's Modal account".
  - Satisfied by p5/deployment: `deploy/modal_app.py` exists and matches design D4 — `ast.parse` clean, header marked "READY, NOT EXERCISED"; live Modal deploy deferred per user instruction (no account, no card, no cost).
- [x] 2.2 Document the vLLM variant for concurrent multi-user serving (when to switch, what changes in `deploy/modal_app.py`, `TELCO_LLM_BASE_URL` value) and the deferral clause (no account → local compose stack is the legitimate demo path) in `docs/adr/0001-modal-serverless-over-free-tier-vms.md` as an amendment. Verify: ADR status updated to amended-with-date, decision text matches deployment delta.
  - Verified 2026-09-19: ADR front matter now `status: amended` + `amended: 2026-09-19`; appended "Amendment (2026-09-19): single-container Ollama runtime; vLLM demoted to variant" matching the deployment delta (incl. the deferral clause).
- [x] 2.3 Reconcile `openspec/changes/add-telco-churn-poc/tasks.md` §6 wording to the amended runtime (Ollama single-container default, vLLM documented variant) so the two changes do not contradict. Verify: `grep -n "vLLM" openspec/changes/add-telco-churn-poc/tasks.md` shows only variant references, no primary-runtime claims.
  - Superseded 2026-09-19: the target change was finalized and archived (`openspec/changes/archive/2026-09-19-add-telco-churn-poc/`, commit f71df95) before this change's apply — archived history is frozen, so no reconciliation edit is possible. The runtime reconciliation landed as the ADR-0001 amendment (task 2.2).
- [x] 2.4 Full verification repeat: pytest + pre-commit green on the deployment-amendment commits. Verify: both commands pass.

## 3. Transparency verification

- [x] 3.1 Write `tests/eval/test_demo_smoke.py` under the `eval_llm` marker: reach the real backend (skip if unreachable), perform one real authenticated chat turn through `ChurnChatPipeline`, write `runs/demo_smoke_report.json` recording model identity, backend, turn latency, and schema validity; a failing backend produces a loud report, never a fabricated payload. Verify: gated run against local Ollama writes the report; unit-mode run skips cleanly.
  - Verified 2026-09-19: gated run passed (85.3 s cold CPU) and wrote `runs/demo_smoke_report.json` with ok=true, schema_validity=true, model/backend/latency recorded; forced-unreachable run skipped cleanly (1 skipped).
- [x] 3.2 Final system verification and user acceptance (user accepted 2026-09-19): full pytest zero-warnings, pre-commit green, then the user drives one real turn through `/demo` against the live local Ollama stack and confirms the answer, drivers, and model identity visible on the page. Verify: `runs/demo_smoke_report.json` exists with a real turn; user confirms acceptance in chat.

## 4. Coordination

- [x] 4.1 Confirm branch placement with the user at apply time (suggested: `p6/demo-surface` created from the p5/deployment lineage so history carries; user decides fold vs. new branch). Verify: user's recorded choice in the session before the first implementation commit.
  - User chose `p6/demo-surface` (created from `p5/deployment` @ f71df95) on 2026-09-19, recorded before the first implementation commit.
